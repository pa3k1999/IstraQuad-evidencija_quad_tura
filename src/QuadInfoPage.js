  ...
  useEffect(() => {
    ...
    getTure(id).then((tureTemp) => {
      ...
      getNapomene(id).then(async (napomeneTemp) => {
        const napomeneFilterTemp = napomeneTemp.sort(
          (a, b) =>
            tureTemp.find((t) => t.id === b.turaId).vrijemePocetka -
            tureTemp.find((t) => t.id === a.turaId).vrijemePocetka
        );
        setNapomeneQuad(napomeneFilterTemp);
        setIsLoadingNapomene(false);
        const vrsteQuadovaTemp = await getVrsteTura();
        setUkupnoVrijeme(
          tureTemp.reduce((sum, t) => sum + vrsteQuadovaTemp.find((vT) => vT.id === t.vrstaTureId).minute, 0)
        );
      });
    });
        ...
        <List style={{ padding: '0' }} dense={true}>
          {isLoadingNapomene
            ...
            : napomeneQuad.map((n) => {
                ...
                return (
                  <NapomenaQuadaListItem
                    key={n.id}
                    nazivTure={tura.naziv}
                    datum={tura.vrijemePocetka.toDate()}
                    napomena={n.napomena}
                  />
                );
              })}
