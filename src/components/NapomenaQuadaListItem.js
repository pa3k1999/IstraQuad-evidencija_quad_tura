    ...
    <ListItem disablePadding>
      <ListItemButton style={{ padding: '0' }}>
        <Stack style={{ width: '100%' }}>
          <Stack justifyContent="space-between" direction="row" style={{ padding: '5px 0', backgroundColor: 'rgba(0, 0, 0, 0.05)', width: '100%' }}>
            <Typography style={{ margin: '0 10px', padding: '0' }}>{nazivTure}</Typography>
            <Typography style={{ margin: '0 10px', padding: '0' }}>{`${datum.getDate()}.${datum.getMonth() + 1}.${datum.getFullYear()}`}</Typography>
          </Stack>
          {napomena ? (
            <>
              <Divider />
